	...
	if(size==4)
	{
		if(rank==0)
		{
			...
			for(int i2=0; i2<2; i2++)
			{
				for(int i3=0; i3<2; i3++)
				{
					...
					MPI_Send(&dbuff,1,MPI_DOUBLE,2,tag1,MPI_COMM_WORLD);
					tag=tag+3;
					tag1=tag1+3;
					fprintf(filekoef,"%e\n",a[i2][i3]);
					//printf("%lf  ",a[i2][i3]);
			...
			while(i<1000);
			fclose(file);
			MPI_Recv(&dbuff,1,MPI_DOUBLE,2,tag1,MPI_COMM_WORLD,&status);
		...
		else if(rank==2)
		{
			...
			for(int i2=0;i2<2;i2++)
			{
				for(int i3=0;i3<2;i3++)
				{
					MPI_Recv(&dbuff,1,MPI_DOUBLE,0,tag1,MPI_COMM_WORLD,&status);
					a[i2][i3]=dbuff;
					tag1=tag1+3;
					//printf("%lf ",a[i2][i3]);
				}
                        ...
                        for(int i2=0;i2<2;i2++)
			{
				...
				tag1=tag1+3;
                        ...
                        FILE  *filekoef1;
			filekoef1 = fopen("/home/sergey/MPIprojects/Method/Method/koef1.dat","w");
                        //writemass1(x,2);                        
                        ...
                        for(int i2=0; i2<2; i2++)
			{
				...
				{
					dbuff=a[i2][i3];
					MPI_Send(&dbuff,1,MPI_DOUBLE,3,tag2,MPI_COMM_WORLD);
					tag2=tag2+3;
					fprintf(filekoef1,"%e\n",a[i2][i3]);
					//printf("%lf  ",a[i2][i3]);
				}
			...
			for(int i2=0; i2<2; i2++)
			{
				...
				fprintf(filekoef1,"%e\n",b[i2]);
                        ...
                        fclose(filekoef1);
			FILE *file1;
			file1 = fopen("/home/sergey/MPIprojects/Method/Method/data1.dat", "w");
			fprintf(file1,"%e  %e 0.0\n", x[0],x[1]);
			int i=0;
                        do
			{
				for(int i2=0; i2<2; i2++)
				...
				//writemass1(epps,2);
				//cout<<epps[0]<<"   "<<epps[1]<<endl;
				MPI_Recv(&dbuff,1,MPI_DOUBLE,3,tag2,MPI_COMM_WORLD,&status);
				next[1]=dbuff;
				fprintf(file1,"%e  %e 0.0\n",next[0],next[1]);
				tag2=tag2+3;
				MPI_Recv(&dbuff,1,MPI_DOUBLE,3,tag2,MPI_COMM_WORLD,&status);
			...
			writemass1(grad,2);
			double rrr=100;
			MPI_Send(&rrr,1,MPI_DOUBLE,0,tag1,MPI_COMM_WORLD);
			tag1=tag1+3;
			//writemass2(a,2);
		...
		else if(rank==3)
		{
			...
			for(int i2=0;i2<2;i2++)
			{
				for(int i3=0;i3<2;i3++)
				{
					MPI_Recv(&dbuff,1,MPI_DOUBLE,2,tag2,MPI_COMM_WORLD,&status);
					a[i2][i3]=dbuff;
					tag2=tag2+3;
					//printf("%lf ",a[i2][i3]);
				}
			...
			do
			{
				...
				epps[1]=abs(x[1]-next[1]);
				dbuff=next[1];
				MPI_Send(&dbuff,1,MPI_DOUBLE,2,tag2,MPI_COMM_WORLD);
				tag2=tag2+3;
				dbuff=epps[1];
				MPI_Send(&dbuff,1,MPI_DOUBLE,2,tag2,MPI_COMM_WORLD);
				tag2=tag2+3;
